...
func main() {
	...
	dbPath := filepath.Join(filepath.Dir(*configPath), "homectl.db")
	...
	// Initialize health check executor and scheduler
	checkExecutor := checks.NewExecutor(netClient)
	healthScheduler := checks.NewScheduler(cfg, checkExecutor, db)
	...
	api.Get("/services", handlers.ServicesHandler(cfg, cacheManager, widgetRegistry, db))
	...
	if err := config.Watch(ctx, *configPath, func(newCfg *config.Config) {
		...
		cfg = newCfg
		healthScheduler.UpdateConfig(newCfg)
