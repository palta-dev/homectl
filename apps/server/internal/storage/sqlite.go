...
// GetHistory retrieves the last 24 hours of check results for a service
func (d *DB) GetHistory(serviceID string) ([]CheckResult, error) {
	rows, err := d.conn.Query(`
		SELECT timestamp, state, latency_ms 
		FROM check_results 
		WHERE service_id = ? AND timestamp > datetime('now', '-24 hours')
	...
	for rows.Next() {
		var r CheckResult
		if err := rows.Scan(&r.Timestamp, &r.State, &r.LatencyMs); err != nil {
			return nil, err
		}
