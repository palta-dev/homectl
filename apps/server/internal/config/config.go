...
func expandEnv(data []byte) []byte {
	return envPattern.ReplaceAllFunc(data, func(match []byte) []byte {
		...
		// If not found in env or secrets, return empty string instead of original pattern
		return []byte("")
	})
}
...
func validate(cfg *Config) error {
	...
	if cfg.Settings.RequestTimeout != "" {
		if _, err := time.ParseDuration(cfg.Settings.RequestTimeout); err != nil {
			return fmt.Errorf("settings: invalid requestTimeout %q: %w", cfg.Settings.RequestTimeout, err)
		}
	}
	...
	for i, g := range cfg.Groups {
		...
		for j, s := range g.Services {
			...
			// Validate Checks
			for k, check := range s.Checks {
				if check.Type != "http" && check.Type != "tcp" && check.Type != "ping" {
					return fmt.Errorf("group[%d].service[%d].check[%d]: invalid type %q", i, j, k, check.Type)
				}
				if check.Timeout != "" {
					if _, err := time.ParseDuration(check.Timeout); err != nil {
						return fmt.Errorf("group[%d].service[%d].check[%d]: invalid timeout %q: %w", i, j, k, check.Timeout, err)
					}
				}
			}
			...
			for k, w := range s.Widgets {
				if w.Type == "" {
					return fmt.Errorf("group[%d].service[%d].widget[%d]: type is required", i, j, k)
				}
			}
		}
	}
