...
// HistoryPoint represents a single point in history
...
func ServicesHandler(cfg *config.Config, cacheManager *cache.Manager, widgetRegistry *widgets.Registry, db *storage.DB) fiber.Handler {
	...
	discoveredGroup := GroupWithStatus{
			...
			for i, svc := range filteredDiscovered {
				discoveredGroup.Services[i] = ServiceWithStatus{
					...
					Tags:         svc.Tags,
					NewTab:       svc.NewTab,
					Status:       getServiceStatus(svc, cacheManager, db),
					IsDiscovered: true,
				}
			}
		...
		for _, group := range cfg.Groups {
			allGroups = append(allGroups, processGroup(group, cacheManager, widgetRegistry, db))
		}
...
func processGroup(group config.Group, cacheManager *cache.Manager, widgetRegistry *widgets.Registry, db *storage.DB) GroupWithStatus {
	...
	for i, svc := range group.Services {
		gResp.Services[i] = processService(svc, cacheManager, widgetRegistry, db)
	}

	return gResp
}

func processService(svc config.Service, cacheManager *cache.Manager, widgetRegistry *widgets.Registry, db *storage.DB) ServiceWithStatus {
...
func getServiceStatus(svc config.Service, cacheManager *cache.Manager, db *storage.DB) ServiceStatus {
	now := time.Now().UTC().Format(time.RFC3339)
	state := "unknown"
	var latency *int64

	// If DB is available, get history and current state from it
	if db != nil {
		history, err := db.GetHistory(svc.Name)
		if err == nil && len(history) > 0 {
			...
			// Map to API history points
			apiHistory := make([]HistoryPoint, len(history))
			for i, p := range history {
				apiHistory[i] = HistoryPoint{
	...
	// Fallback for services without DB history (e.g. newly discovered)
