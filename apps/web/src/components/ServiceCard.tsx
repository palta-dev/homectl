...
export function ServiceCard({ service }: ServiceCardProps) {
  ...
  const generatePath = (history: any[], isClosed: boolean) => {
    ...
    const points = history.map((h, i) => {
      ...
      const y = height - padding - ((h.latencyMs - minLatency) / range) * usableHeight;
      return { x, y };
    });
            ...
            <div className="spark">
              <svg viewBox="0 0 170 66" preserveAspectRatio="none">
                ...
                {status.history && status.history.length > 1 ? (
                  <>
                    <path
                      d={generatePath(status.history, true)}
                      fill={`url(#grad-${name})`}
                    />
                    <path
                      d={generatePath(status.history, false)}
                      fill="none"
                      stroke="var(--green)"
                      strokeWidth="2"
                    />
